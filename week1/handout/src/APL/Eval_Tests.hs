...
import APL.Eval (EvalError (DivisionByZero, NegativeExponent, InvalidOperandsType, NonBooleanCondition), Val (ValBool, ValInt), eval)
      ...
      testGroup
        ...
        [ testCase "If statement with non boolean condition" $
            assertBool "should return a non boolean condition error" $
              Right NonBooleanCondition == eval (If (CstInt 0) (CstInt 0) (CstInt 1)),
          testCase "Equality between different types" $
            assertBool "should return an invalid operands type error" $
              Right InvalidOperandsType == eval (CstInt 2 `Eql` CstBool True)
        ]
    ]
