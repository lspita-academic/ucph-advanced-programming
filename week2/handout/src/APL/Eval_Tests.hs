...
import APL.AST (Exp (..))
import APL.Eval (Error, Val (..), envEmpty, eval, runEval)
...
eval' e = runEval (eval e) envEmpty
...
tests =
  testGroup
    ...
    [ testCase "Add" $
      ...
      --
      testCase "ForLoop (p)" $
        eval' (ForLoop ("p", CstInt 3) ("i", CstInt 5) (Add (Var "p") (CstInt 1)))
          @?= Right (ValInt 8),
      --
      testCase "ForLoop (i)" $
        eval' (ForLoop ("p", CstInt 0) ("i", CstInt 5) (Add (Var "p") (Var "i")))
          @?= Right (ValInt 10),
      --
      testCase "ForLoop (initial)" $
        eval' (ForLoop ("p", CstBool True) ("i", CstInt 0) (CstBool False))
          @?= Right (ValBool True),
      --
      testCase "ForLoop (let i)" $
        eval' (ForLoop ("p", CstInt 0) ("i", CstInt 5) (Let "i" (CstInt 10) (Add (Var "p") (CstInt 1))))
          @?= Right (ValInt 5),
      --
      testCase "ForLoop (non-integral loop bound)" $
        eval' (ForLoop ("p", CstBool True) ("i", CstBool True) (CstBool False))
          @?= Left "Non-integral loop bound",
      --
      testCase "Lambda & Apply" $
        eval' (Apply (Let "y" (CstInt 2) (Lambda "x" (Add (Var "x") (Var "y")))) (CstInt 3)) @?= Right (ValInt 5),
      --
      testCase "Lambda & Apply (fact)" $
        eval' (Apply fact (CstInt 6)) @?= Right (ValInt 720),
      --
      testCase "Apply evaluation order" $
        eval' (Apply (Var "x") (Div (CstInt 4) (CstInt 0)))
          @?= Left "Unknown variable: x",
      --
      testCase "TryCatch (pass)" $
        eval' (TryCatch (CstBool True) (CstBool False)) @?= Right (ValBool True),
      --
      testCase "TryCatch (eval order)" $
        eval' (TryCatch (CstInt 42) (Div (CstInt 1) (CstInt 0))) @?= Right (ValInt 42), --
        --
      testCase "TryCatch (fail)" $
        eval' (TryCatch (Div (CstInt 1) (CstInt 0)) (CstInt 42)) @?= Right (ValInt 42)
    ]
